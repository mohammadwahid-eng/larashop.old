			...
			<form action="{{ route('admin.attributes.values.update', [$attribute, $value]) }}" method="POST" class="card">
				...
				<div class="card-body pb-0">
					<div class="form-group">
						...
						<input type="text" class="form-control @error('name') error @enderror" id="name" name="name" value="{{ old('name') ?? $value->name }}" data-slugify="#slug" required>
						@error('name')
					...
					<div class="form-group">
						...
						<input type="text" class="form-control @error('slug') error @enderror" id="slug" name="slug" value="{{ old('slug') ?? $value->slug }}" required>
						@error('slug')
					...
					<div class="form-group">
						...
						<textarea name="description" id="description" class="form-control @error('description') error @enderror">{{ old('description') ?? $value->description }}</textarea>
						@error('description')
